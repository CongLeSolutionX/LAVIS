...
class RunnerIter(RunnerBase):
    ...
    def _progress(self, cur_iters):
        return f"{self.cur_epoch}_iters={cur_iters}"
    ...
    def train(self):
        ...
        for start_iters in range(
            ...
            if not self.evaluate_only:
                logging.info(
                    f"Start training, max_iters={self.max_iters}, in total {int(self.max_iters / self.iters_per_inner_epoch)} inner epochs."
                )
    ...
    def _save_checkpoint(self, cur_iters, is_best=False):
        ...
        save_to = os.path.join(
            self.output_dir, f'checkpoint_{"best" if is_best else cur_iters}.pth'
        )
        logging.info(f"Saving checkpoint at iters {cur_iters} to {save_to}.")
        ...
        if self._dataloaders is None:
            ...
            if dataset_ratios is None:
                ...
                if missing_keys := [
                    ...
                    raise ValueError(
                        f"Datasets with the following split names are not found: {missing_keys}"
                    )
                ...
                if unexpected_keys := [
                    ...
                    raise ValueError(
                        f"Datasets with the following split names are not expected: {unexpected_keys}"
                    )
